...
let rec generate (tctx : T_Ctx.t) (expr : EExpr.t) : t =
  ...
  | EExpr.BinOpt (Operator.LogicalAnd, e1, e2) ->
    ...
    And (ce1, ce2)
  | EExpr.BinOpt (Operator.LogicalOr, e1, e2) ->
    let ce1 = generate tctx e1 in
    let ce2 = generate tctx e2 in
    Or (ce1, ce2)
...
type element_t = EExpr.t * constraint_t
...
let create_constraint (expr : EExpr.t) (tcstr : EType.t) (isNeq : bool)
...
let eval_constraint (ttar : EType.t) (cstr : constraint_t) : EType.t list =
  let _terr ttar cstr =
  ...
  let rec _runtime_cmp_f t =
    match t with
    | EType.RuntimeType Type.TypeType -> true
    | EType.RuntimeType _ -> t = cstr.tcstr
    | _ -> _runtime_cmp_f (EType.to_runtime t)
  in
  if cstr.isTypeof then
    ...
    let tlit = EType.unfold_type false ttar in
    let tunfolded = EType.unfold_type true ttar in
    let tremoved =
      List.filter (fun t -> List.mem t tlit) (EType.tlst cstr.tcstr)
    in
    let tkeep = List.filter (fun t -> not (List.mem t tremoved)) tunfolded in
    EType.fold_type tkeep
  else
    let isTypeable_f = T_Typing.is_typeable ttar in
    let tconstraint = List.filter isTypeable_f (EType.tlst cstr.tcstr) in
...
let apply_clause (tctx : T_Ctx.t) (clause : t) : unit =
  ...
  if clause <> NoConstraint then
    ...
    let tconstraint = T_Narrowing.narrow_type (EType.UnionType tevals) in

...
let parseJS (prog : Prog.t) (heap : Heap.t) (str : string) : Val.t =
...
let loadInitialHeap (prog : Prog.t) (heap : Heap.t) (file : string) : Val.t =
  Val.Loc (Parsing_heap_utils.parse_and_update heap file)

let execute (prog : Prog.t) (heap : Heap.t) (f : string) (vs : Val.t list) :
    Val.t =
  match (f, vs) with

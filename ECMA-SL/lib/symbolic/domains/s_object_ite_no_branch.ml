...
module M : S_object_intf.S = struct
  type vt = Expr.t
  type pct = Expr.t
  type encoded_pct = Encoding.Expression.t

  module ExprHash : Hashtbl_intf.Key with type t = Expr.t = struct
    type t = Expr.t
    ...
    let str_obj =
      List.fold_left (Expr_Hashtbl.to_alist o.symbolic_fields)
        ~init:(str_obj ^ ", ") ~f:(fun acc (key, data) ->
          acc
          ^ Printf.sprintf "\"$symb_%s\": %s, " (Expr.str key) (printer data))
    in
    str_obj ^ " }"
  ...
  let create_not_pct (l : (pct * Expr.t) list) (key : pct) :
      encoded_pct list =
  ...
  let set (o : t) (key : vt) (data : Expr.t) (solver : Batch.t)
    ...
    | _ ->
        ...
        if temp = 0 then (
          ...
          let new_pc =
            create_not_pct (concrete_conds @ symbolic_conds) key
          in
    ...
    | Expr.Val (Val.Str key_s) -> (
        ...
        | None ->
            if Expr_Hashtbl.length o.symbolic_fields = 0 then [ (o, [], None) ]
              ...
              let ite, _ = create_ite lst key in
              [ (clone o, [], Some ite) ])
    | _ -> (
        ...
        | None ->
            ...
            in
            let ite_symb, _ = create_ite (symb_lst @ concrete_lst) key in
  ...
  let delete (o : t) (key : Expr.t) (solver : Batch.t) (pc : encoded_pct list)
    ...
    | _ -> (
        ...
        | None ->
            ...
            let new_pc =
              create_not_pct (symbolic_list @ concrete_list) key
            in
            if Batch.check solver (new_pc @ pc) then
              let o' = clone o in
              (o', new_pc) :: rets
  ...
  let get_fields (o : t) : Expr.t list =
    let ret =
      List.map (Hashtbl.keys o.concrete_fields) ~f:(fun f ->
          Expr.Val (Val.Str f))
    in

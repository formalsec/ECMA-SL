...
let stmts b msg = match b with Cont stmts -> Ok stmts | _ -> Error msg
...
let step (c : State.config) : (State.config list, string) Result.t =
  ...
  | Exception err ->
      ...
      return [ update c (Error (Some (Val (Val.Str err)))) state pc ]
  | Print e ->
  ...
  | Stmt.AssignInObjCheck (x, e_field, e_loc) ->
      let* locs = loc (reduce_expr ~at:s.at store e_loc) in
  ...
  | Stmt.AssignObjToList (x, e) ->
      let f h l pc' =
        ...
        update c
          ...
          pc'
      in
      let* locs = loc (reduce_expr ~at:s.at store e) in
      return
        (List.fold locs ~init:[] ~f:(fun accum (cond, l) ->
             match cond with
             | None ->
  ...
  | Stmt.SymStmt (SymStmt.Maximize (x, e)) ->
      ...
      in
      return [ update c (Cont (List.tl_exn stmts)) (heap, store', stack, f) pc ]

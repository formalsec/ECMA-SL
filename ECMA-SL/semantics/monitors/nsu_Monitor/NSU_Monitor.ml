  ...
  let rec eval_small_step (m_state : state_t) (tl : sl SecLabel.t) :
    ...
    | ReturnLab e -> (
      ...
      | Intermediate (pc', ssto', x) ->
        eval_small_step (scs', sheap, ssto', pc') (SecLabel.UpgVarLab (x, lvl_f))
    ...
    | AssignCallLab (params, exp, x, _f) -> (
      ...
      | Some x_lvl ->
        if SL.leq (check_pc pc) x_lvl then
          let scs' =
            SecCallStack.push scs (SecCallStack.Intermediate (pc, ssto, x))
          in
      ...
      | None -> (
        ...
        | Some lev_struct ->
          if SL.leq lev_ctx lev_struct then
            if
              SecHeap.new_sec_prop sheap loc field lev_ctx

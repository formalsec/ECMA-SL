type base_t = Var of string | Loc of string
...
let parse_base (s : string) : base_t =
  match Loc.parse_loc s with Some l -> Loc l | _ -> Var s
...
let rec inspector (heap : Val.t Heap.t) (sto : Val.t Store.t) : unit =
  ...
  | Some ShowStore ->
      ...
      f ()
  | Some ShowHeap ->
      Printf.printf "%s" (Heap.to_string heap Val.str);
      f ()
  | Some Continue -> ()
  | None ->

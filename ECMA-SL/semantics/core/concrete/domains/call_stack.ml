...
let pp_frame (ppf : Fmt.t) (frame : 'store frame) : unit =
  ...
  Fmt.fmt ppf "'%s' in %a" (Func.name' func) pp_loc stmt.at
...
let pp (ppf : Fmt.t) (stack : 'store t) : unit =
  ...
  else fmt ppf "{ %a }" (pp_lst !>" <- " pp_binding) stack

let pp_tabular (ppf : Fmt.t) (stack : 'store t) : unit =
  let open Fmt in
  let pp_trace ppf frame = fmt ppf "\nCalled from %a" pp_frame frame in
  match stack with
  | [] -> fmt ppf "Empty call stack"
  | frame :: stack' ->
    fmt ppf "%a%a" pp_frame frame (pp_lst !>"" pp_trace) stack'

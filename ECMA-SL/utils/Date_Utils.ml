...
let is_timezone str i =
    Printf.printf "debug 42: is_timezone %s %d %b \n" str i (String.get str i <= 'Z' && String.get str i >= 'A');
    String.get str i <= 'Z' && String.get str i >= 'A'



let parse_segment (str : string) (i : int) (prev : char) (delim : char) : (float * int) option = 
    Printf.printf "debug 42: parse_segment %s, %d %c %c\n" str i prev delim; 
    let len = String.length str in
    if (String.length str = (i + 1)) && (String.get str i = prev)  then
        None
    else
        let rec loop (chrs : char list) (j : int) = 
            if (j >= len) || ((String.get str j) = delim)
                then Some (chrs, j)
                else 
...
let parse_sth (str : string) (fs : (string -> int -> (float * int) option) list) : (float list * int) option =
    ...
    List.fold_left (fun ac f ->  
        ...
        | Some (ns, i) -> 
            ...
            (match ret with 
            | None when (is_final_index str i || is_timezone str i) -> (Printf.printf "debug 42: parse_sth in first case\n"; 
                                            Some (ns @ [ Float.nan ], i))
            | None ->(Printf.printf "debug 42: parse_sth in none case\n";  None) 
            | Some (n, i') -> (Printf.printf "debug 42: parse_sth in last case\n"; Some (ns @ [ n ], i')))
    ) (Some ([], 0)) fs 

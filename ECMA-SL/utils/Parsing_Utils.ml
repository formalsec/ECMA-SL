...
let rec resolve_imports
      (to_resolve : string list list)
      (resolved   : StrSet.t)
      (path       : string list)
      (funcs      : E_Func.t list)
      (macros     : E_Macro.t list) : E_Func.t list * E_Macro.t list =
  match (to_resolve, path) with
  | ([], _) -> funcs, macros
  | ([] :: lst, file :: rest_path) ->
    let resolved =  StrSet.add file resolved in
    resolve_imports lst resolved rest_path funcs macros
  | ((file :: files)::_, _) ->
    if (StrSet.mem file resolved)
    then resolve_imports [files] resolved path funcs macros
          ...
          then failwith "Error resolving imports: Cyclic dependency"
            ...
            resolve_imports
              ...
              resolved
              (file :: path)
              (cur_prog_funcs @ funcs)
              (cur_prog_macros @ macros)
          ))
...
let resolve_prog_imports (prog : E_Prog.t) : E_Prog.t =
  ...
  let (total_funcs, total_macros) = resolve_imports  [ E_Prog.get_imports prog ] StrSet.empty [ E_Prog.get_file_name prog ] (E_Prog.get_funcs prog) (E_Prog.get_macros prog) in

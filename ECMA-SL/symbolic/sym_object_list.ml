...
module M : sig
  ...
  let obj_record_to_string (o_r : obj_record) : string =
    let aux e =
      match e with
      | Some v -> Format.asprintf "%a" Value.Pp.pp v
      | None -> "deleted"
    in
  ...
  let rec get_prop_aux ?(default_val = Value.Val (Val.Bool false)) (o : t)
    ...
    (lst : (value * value) list list) (solver : Batch.t) (pc : encoded_pct list)
    ...
    | o_rec :: o_rest -> (
      ...
      | [], None -> get_prop_aux ~default_val o_rest prop get_val lst solver pc
  ...
  let get_prop ?(default_val = Value.Val (Val.Bool false)) (o : t)
    (prop : value) (solver : Batch.t) (pc : encoded_pct list)
  ...
  let has_field (obj : t) (k : value) (solver : Batch.t) (pc : encoded_pct list)
    : value =

...
let create_ite (lst : (pct * pct) list) (key : Expr.t) (pc : encoded_pct list)
  ...
  let not_new_encoded = Translator.translate not_new in

  if Batch.check solver (not_new_encoded :: pc) then 
    (ite, Some new_pc), Some (undef, Some not_new)
  else 
    (ite, Some new_pc), None
...
let get_possible_fields_symbolic (o : t) (key : Expr.t)
    (solver : Batch.t) (pc : encoded_pct list) (store : S_store.t) :
    (pct * pct) list =
...
let set (o : t) (key : vt) (data : Expr.t) (solver : Batch.t)
  ...
  | _ ->
      ...
      if temp = 0 then (
        ...
        let symbolic_conds =
          get_possible_fields_symbolic o key solver pc store
        in
        let concrete_conds =
          get_possible_fields_concrete o key solver pc store
        in

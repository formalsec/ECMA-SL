...
let usage = "Usage: " ^ name ^ " -i <path> -mode <c/p> -o <path> [-v|-s] -h <path> [--parse]"

let argspec = Arg.align
[
  "-i", Arg.String (fun f -> Flags.file := f), " input file";
  "-h", Arg.String (fun f -> Flags.heap_file := f),
  ...
  "-mode", Arg.String (fun m -> Flags.mode := m ), 
    " mode to run: c - Core / p - Plus ";
  "--parse", Arg.Set Flags.parse, " parse to JSON";
  "--version", Arg.Unit (fun () -> banner (); exit 0), " show version"
]
...
let compile_from_plus_to_core () : unit =
  ...
  if !Flags.output <> "" then File_Utils.burn_to_disk !Flags.output (Prog.str c_prog)
...
let core_interpretation (prog : Prog.t) : exit_code =
  ...
  let v, heap = CoreInterp.eval_prog prog (!Flags.output, !Flags.mon, !Flags.verbose) "main" in
  if !Flags.heap_file <> "" then File_Utils.burn_to_disk !Flags.heap_file (Heap.str_with_global heap);
  (match v with
   | Some z -> (match z with
...
let run () =
  ...
  print_string "=====================\n\tECMA-SL\n=====================\n";
  if !Flags.silent then Logging.set_silent ();       (* Disable logging (when using "print_endline" and/or "print_string") *)
  let code : exit_code =
  if (!Flags.file = "" && !Flags.mode = "" && !Flags.output = "") then (print_string "No option selected. Use -h"; FAILURE)
  else if (!Flags.file = "") then (print_string "No input file. Use -i\n=====================\n\tFINISHED\n=====================\n"; FAILURE)
  else if (!Flags.mode = "") then (print_string "No mode selected. Use -mode\n=====================\n\tFINISHED\n=====================\n"; FAILURE)
  else if (!Flags.mode = "ci") then (print_string "======================= CORE =======================\n";
                               let prog_contents = Parsing_Utils.load_file !Flags.file in
                               let prog = Parsing_Utils.parse_prog prog_contents in
  ...
  else if (!Flags.mode = "ic") then (print_string "======================= Inlining Monitor =======================\n";

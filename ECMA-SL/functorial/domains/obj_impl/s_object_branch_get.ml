...
let set_concrete_field (o : t) (key : value) (data : value) : unit =
  ...
  | _ -> Format.kasprintf failwith "bad field: %a" V.Pp.pp key

let has_concrete_key (o : t) (key : string) : bool =
  Hashtbl.mem o.concrete_fields key

let concrete_to_list (o : t) : (pct * value) list =
  Hashtbl.fold
...
let get_concrete_field (o : t) (key : string) : value option =
  Hashtbl.find_opt o.concrete_fields key
...
let get_possible_fields_concrete (o : t) (key : value) (solver : Solver.t)
  (pc : encoded_pct list) : (pct * pct) list =
  Hashtbl.fold
    (fun k d acc ->
...
let get_possible_fields (o : t) (key : value) (solver : Solver.t)
  (pc : encoded_pct list) : (pct * pct) list =
  let conc = get_possible_fields_concrete o key solver pc in
  let symb = get_possible_fields_symbolic o key solver pc in
  conc @ symb
...
let has_field (o : t) (k : value) : value =
  let open Val in
  if
    Hashtbl.length o.concrete_fields = 0 && Hashtbl.length o.symbolic_fields = 0
  then Val (Bool false)
...
let get (o : t) (key : value) (solver : Solver.t) (pc : encoded_pct list) :
...
let delete (o : t) (key : value) (solver : Solver.t) (pc : encoded_pct list) :
...
let get_fields (o : t) : value list =
  let ret =
    Seq.map (fun f -> V.Val (Val.Str f)) (Hashtbl.to_seq_keys o.concrete_fields)
  in

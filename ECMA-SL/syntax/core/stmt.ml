...
let rec js (stmt : t) : string =
  ...
  | AssignInObjCheck (x, e_o, e_f) ->
    Printf.sprintf "%s = %s.hasOwnProperty( %s )" x (Expr.js e_o) (Expr.js e_f)
  | AssignObjToList (x, e) ->
...
let rec to_json (stmt : t) : string =
  ...
  | Assign (v, exp) ->
    Printf.sprintf "{\"type\" : \"assign\", \"lhs\" :  \"%s\", \"rhs\" :  %s}" v
      (Expr.to_json exp)
  | If (e, s1, s2) ->
    Printf.sprintf "{\"type\" : \"condition\", \"expr\" : %s, \"then\" : %s %s}"
  ...
  | FieldDelete (e, f) ->
    Printf.sprintf "{\"type\" : \"fielddelete\", \"obj\" : %s, \"field\" : %s}"

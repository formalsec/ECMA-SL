...
let rec pp (ppf : Fmt.t) (e : t) : unit =
  ...
  | Call (fe, es, ferr) ->
    let pp_catch ppf ferr = fprintf ppf " catch %a" Id.pp ferr in
    fprintf ppf "%a(%a)%a" pp fe (pp_lst ", " pp) es (pp_opt pp_catch) ferr
  | ECall (fn, es) -> fprintf ppf "extern %a(%a)" Id.pp fn (pp_lst ", " pp) es
  | NewObj flds ->
    let pp_fld ppf (fn, fe) = fprintf ppf "%a: %a" Id.pp fn pp fe in
    if List.length flds = 0 then pp_str ppf "{}"
    else fprintf ppf "{ %a }" (pp_lst ", " pp_fld) flds
  | Lookup (oe, fe) -> fprintf ppf "%a[%a]" pp oe pp fe
  | Curry (fe, es) -> fprintf ppf "{%a}@(%a)" pp fe (pp_lst ", " pp) es
  | Symbolic (t, e') -> fprintf ppf "se_mk_symbolic(%a, %a)" Type.pp t pp e'

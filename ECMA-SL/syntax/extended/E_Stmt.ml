...
type t = Skip
       ...
       | Block       of t list
       | If          of E_Expr.t * t * t option * metadata_t list * metadata_t list    (** "If" and "Else" metadata *)
       | EIf         of (E_Expr.t * t * metadata_t list) list * (t * metadata_t list) option
       | While       of E_Expr.t * t
       | ForEach     of string * E_Expr.t * t * metadata_t list * (string * string) option
       | Return      of E_Expr.t
       ...
       | Switch      of E_Expr.t * (E_Expr.t * t) list * t option * string  (** metadata; just "table caption" for now. *)
       | Lambda      of string * string * string list * string list * t

let is_basic (s : t) : bool = match s with
...
let rec str (stmt : t) : string =
  ...
  | If (e, s1, s2, _, _)       -> (let v = "if (" ^ E_Expr.str e ^ ") " ^ str s1 in
                                   match s2 with
                                   | None   -> v
                                   | Some s -> v ^ " else " ^ str s)
  | EIf (ifs, final_else)      -> (let ifs' =
                                     List.map (fun (e, s, _) ->
                                         Printf.sprintf "if (%s) %s" (E_Expr.str e) (str s)) ifs in
  ...
  | ForEach (x, exp, s, _, _)  -> Printf.sprintf "foreach (%s, %s) %s" x (E_Expr.str exp) (str s)
  ...
  | Switch (e, cases, so, _)   -> Printf.sprintf "switch (%s) { %s %s }" (E_Expr.str e) (str_cases cases) (str_o so)
...
let rec map
  ...
  let s' =
    ...
    | If (e, s1, s2, m_i, m_e)    -> If (fe e, (map ~fe f s1), Option.map (map ~fe f) s2, m_i, m_e)
    | EIf (ifs, final_else)       -> EIf (f_if_elses ifs, Option.map (fun (s, m) -> ((map ~fe f s), m)) final_else)
    ...
    | Switch (e, cases, so, meta) -> Switch (fe e, f_cases cases, Option.map (map ~fe f) so, meta)
...
let rec to_list (is_rec : (t -> bool)) (f : (t -> 'a list)) (s : t) : 'a list =
  ...
  if (not (is_rec s)) then ret else (
    let ret_rec =
      match s with
      | Skip | Print _ | Wrapper _ | Assign _
      ...
      | Switch(_, cases, so, _) -> f_stmts ((f_cases cases) @ (Option.map_default (fun x -> [x]) [] so)) in
    ret @ ret_rec
  )

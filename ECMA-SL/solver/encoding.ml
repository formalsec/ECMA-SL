...
let rm = Z3.FloatingPoint.mk_const ctx (Z3.Symbol.mk_string ctx "rm") (Z3.FloatingPoint.RoundingMode.mk_sort ctx)
...
(*  Real/FPA operations  *)
let mk_const      = match_enc "mk_const" (Z3.Arithmetic.Real.mk_const ctx) (fun (s : Z3.Symbol.symbol) -> Z3.FloatingPoint.mk_const ctx s fp_sort)
let mk_num_i      = match_enc "mk_num_i" (Z3.Arithmetic.Real.mk_numeral_i ctx) (fun i -> Z3.FloatingPoint.mk_numeral_i ctx i fp_sort)
let mk_num_s      = match_enc "mk_num_s" (Z3.Arithmetic.Real.mk_numeral_s ctx) (fun s -> Z3.FloatingPoint.mk_numeral_s ctx s fp_sort)
...
let mk_add = match_enc "mk_add" (fun e1 e2 -> Z3.Arithmetic.mk_add ctx [e1; e2]) (fun e1 e2 -> Z3.FloatingPoint.mk_add ctx rm e1 e2)
let mk_sub = match_enc "mk_sub" (fun e1 e2 -> Z3.Arithmetic.mk_sub ctx [e1; e2]) (fun e1 e2 -> Z3.FloatingPoint.mk_sub ctx rm e1 e2)
let mk_mul = match_enc "mk_mul" (fun e1 e2 -> Z3.Arithmetic.mk_mul ctx [e1; e2]) (fun e1 e2 -> Z3.FloatingPoint.mk_mul ctx rm e1 e2)
let mk_div = match_enc "mk_div" (fun e1 e2 -> Z3.Arithmetic.mk_div ctx  e1  e2 ) (fun e1 e2 -> Z3.FloatingPoint.mk_div ctx rm e1 e2)

(*  Integer operations  *)
let mk_int_i = Z3.Arithmetic.Integer.mk_numeral_i ctx
let mk_int_s = Z3.Arithmetic.Integer.mk_numeral_s ctx

let mk_add_i = (fun e1 e2 -> Z3.Arithmetic.mk_add ctx [e1; e2])
let mk_sub_i = (fun e1 e2 -> Z3.Arithmetic.mk_sub ctx [e1; e2]) 
let mk_mul_i = (fun e1 e2 -> Z3.Arithmetic.mk_mul ctx [e1; e2])
let mk_div_i = (fun e1 e2 -> Z3.Arithmetic.mk_div ctx  e1  e2 )
...
let z3_esl_literal_sort, esl_lit_operations =
  ...
  let esl_num_constructor =
    Z3.Datatype.mk_constructor ctx (mk_string_symb "Num") (mk_string_symb "isNum")
      [(mk_string_symb "nValue")] [Some numbers_sort] [0] in
  let esl_string_constructor =
    Z3.Datatype.mk_constructor ctx (mk_string_symb "String") (mk_string_symb "isString")
...
let binop_numbers_to_numbers 
      (mk_op : ZExpr.expr -> ZExpr.expr -> ZExpr.expr)  
      (e1 : ZExpr.expr)
      (e2 : ZExpr.expr) : ZExpr.expr =
...
let binop_booleans_to_booleans 
      (mk_op : ZExpr.expr list -> ZExpr.expr)  
      (e1 : ZExpr.expr)
      (e2 : ZExpr.expr) : ZExpr.expr =
...
let encode_binop (op : Oper.bopt) (e1 : ZExpr.expr) (e2 : ZExpr.expr) : ZExpr.expr = 
  match op with 
  ...
  | Equal   -> ZExpr.mk_app ctx esl_lit_operations.bool_constructor [ (Boolean.mk_eq ctx e1 e2) ]
...
let encode_triop (op : Oper.topt) (e1 : ZExpr.expr) (e2 : ZExpr.expr) (e3 : ZExpr.expr) : ZExpr.expr = 
...
let check_unsat (es : Expr.t list) : bool =
  ...
  try 
    ...
    let es'' = List.map (fun e -> ZExpr.mk_app ctx esl_lit_operations.bool_accessor [ e ]) es' in

...
let rec read_primitive_type (p_type : Yojson.Basic.t) : Val.t =
  ...
  | `String s -> (let loc_prefix = "$loc_global_" in
...
let read_json_obj (json_obj : Yojson.Basic.t) : Object.t =
  match json_obj with
  | `Assoc fvs -> (let obj = Object.create() in
                   (List.iter (
...
let parse_and_update (heap : Heap.t) (js_file : string) : string =
  ...
  let heap_in_file, global_loc = (
    match data with
    | `Assoc heap_global -> (
        if List.length heap_global <> 2
        then invalid_arg "Unexpected Heap length: expecting a JSON object with two properties."
        else (List.hd heap_global, List.nth heap_global 1))
    | _                  -> invalid_arg "Unexpected heap file format. Expecting a JSON object."
  ) in
  if (fst heap_in_file) <> "heap" then invalid_arg "Heap wasn't found in heap file.";
  if (fst global_loc) <> "global" then invalid_arg "Location of the global object wasn't found in heap file.";

...
let build_pat_metadata (metadata : Val.t list) (params_alternatives : (string * string) list) : t =
  ...
  let production_text =
    ...
    | None         -> ""
    | Some (Str s) -> s
    | _            -> invalid_arg "Unexpected metadata value type for section pre-text (list index 2): expecting a String value" in
  let post =
    match List.nth_opt metadata 3 with
    | None         -> ""
    | Some (Str s) -> s
    | _            -> invalid_arg "Unexpected metadata value type for section post-text (list index 3): expecting a String value" in
  let production_name =
    match List.nth_opt metadata 4 with
    | None         -> Some ""
    | Some (Null)  -> None
    | Some (Str s) -> Some s
    | _            -> invalid_arg "Unexpected metadata value type for section name (list index 4): expecting a String value or Null" in
  {
    production_number;
    pre;
    production_text;

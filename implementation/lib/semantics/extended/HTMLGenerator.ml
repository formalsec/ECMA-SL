...
let prefixes_match (level : int) (f1 : E_Func.t) (f2 : E_Func.t) : bool =
  ...
  | Some m1, Some m2 ->
    ...
    if (((List.length sec_number1_split) < level) || ((List.length sec_number2_split) < level))
    then false
    else
...
let group_by_level (level : int) (funcs : (E_Func.t * E_Func.t list) list) : ((E_Func.t * E_Func.t list) list) list =
  List.rev (
    List.fold_left (
      fun acc (f, fs) ->
        match acc with
        ...
        | ((f', fs') :: fss) :: fsss when prefixes_match level f f' ->
          (((f', fs') :: fss) @ [(f, fs)]) :: fsss
        | ((f', fs') :: fss) :: fsss -> [ (f, fs) ] :: ((f', fs') :: fss) :: fsss
    ) [] funcs
  )
...
let is_subsec (level : int) (f1 : E_Func.t) (f2 : E_Func.t) : bool =
  ...
  | Some m1, Some m2 ->
    ...
    let sec_number1_split = String.split_on_char '.' sec_number1 in
    let sec_number2_split = String.split_on_char '.' sec_number2 in
    ((List.length sec_number1_split) < (List.length sec_number2_split)) && ((List.length sec_number1_split) < level)
  | _ -> false


let group_head (level : int) (group : (E_Func.t * E_Func.t list) list) : E_Func.t option * ((E_Func.t * E_Func.t list) list) =
  match group with
...
let rec create_std (level : int) (funcs : E_Func.t option * (E_Func.t * E_Func.t list) list) : std_t =
  match funcs with
...
let create_std_aux (funcs : E_Func.t list) : (E_Func.t * E_Func.t list) list =
  ...
  List.rev (
    List.fold_left (
      fun (acc : (E_Func.t * E_Func.t list) list) (f' : E_Func.t) : (E_Func.t * E_Func.t list) list ->
        let (f, fs, fss) =
          (match acc with
           | ((f, fs) :: fss) -> (f, fs, fss)
           | _                -> invalid_arg "Error in \"HTMLGenerator.create_std_aux\": \"acc\" is invalid.") in
        (match E_Func.get_metadata f' with
         | None ->
           invalid_arg (
             Printf.sprintf
               "Error in \"HTMLGenerator.create_std_aux\": function \"%s\" has no metadata."
               (E_Func.get_name f')
           )
         | Some m ->
        ...
        | ObjPat (_, Some meta) ->
          ...
          let func_meta =
            E_Func_Metadata.build_func_metadata
              ...
              (E_Pat_Metadata.get_meta_params meta) in
          let func_name = "dummy" in
          let newfunc = E_Func.create
              (Some func_meta)
              func_name
              []
              stmt in
          Hashtbl.replace new_funcs (E_Pat_Metadata.get_production_number meta) newfunc
